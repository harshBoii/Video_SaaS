  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL(`/admin/integrations?error=slack_denied`, process.env.NEXT_PUBLIC_APP_URL)
      );
    }
    ...
    return NextResponse.redirect(
      new URL(
        `/admin/integrations/slack/select-channel?success=true`,
    ...
    return NextResponse.redirect(
      new URL(
        `/admin/integrations?error=slack_failed&message=${encodeURIComponent(error.message)}`,
        process.env.NEXT_PUBLIC_APP_URL
      )
    );
  }
