...
export default function PublicVersionComparison({ videoId, shareToken, versionIds, onClose }) {
  ...
  }, [videoId, shareToken, versionIds]);

  const loadComparison = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/public/video/${shareToken}/versions/compare`, {
        method: 'POST',
    ...
    <div className="flex flex-col px-4 py-3 bg-gradient-to-r from-gray-800 to-gray-900 border-t border-gray-700">
      <div className="flex items-center gap-3">
        <motion.button whileHover={{ scale: 1.1}} whileTap={{ scale: 0.95 }}
          onClick={() => player.handleSeek(-10)} disabled={!player.ready}
          className="p-2 rounded-full bg-gray-700 hover:bg-gray-600 text-white disabled:opacity-50">
          <SkipBack className="w-5 h-5"/>
        </motion.button>
        <motion.button whileHover={{ scale: player.ready ? 1.15 : 1 }} whileTap={{ scale: player.ready ? 0.9 : 1 }}
          onClick={player.handlePlayPause} disabled={!player.ready}
        ...
        >
          {!player.ready ? <Loader2 className="w-6 h-6 animate-spin" /> : player.playing ? <Pause className="w-6 h-6" /> : <Play className="w-6 h-6" />}
        </motion.button>
        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
          onClick={() => player.handleSeek(10)} disabled={!player.ready}
          className="p-2 rounded-full bg-gray-700 hover:bg-gray-600 text-white disabled:opacity-50">
        ...
        <motion.button whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }}
          onClick={player.handleVolumeToggle} disabled={!player.ready}
    ...
    >
      ...
      >
        ...
        <motion.button whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }}
          onClick={onClose} className="p-2 hover:bg-gray-800 rounded-lg transition-colors">
          <X className="w-6 h-6 text-white" />
        </motion.button>
      </motion.div>

      <div className="flex-1 flex overflow-hidden">
        <motion.div initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }}
          ...
          <div className="p-4 bg-gradient-to-br from-gray-800 to-gray-900 border-b border-gray-700">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${
                version1.isActive ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-gray-700 text-gray-400 border border-gray-600'
              }`}>
        ...
        <motion.div initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }}
          ...
          <div className="p-4 bg-gradient-to-br from-gray-800 to-gray-900 border-b border-gray-700">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${
                version2.isActive ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-gray-700 text-gray-400 border border-gray-600'
              }`}>
