  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/api/integrations/teams/setup', {
        ...
        credentials: 'include',
        body: JSON.stringify({ companyId, webhookUrl, channelName }),
      });

      const data = await response.json();

      if (data.success) {
        router.push('/integrations?success=teams_connected');
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-50 via-slate-50 to-stone-100 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 relative z-10">
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-zinc-900">Setup Microsoft Teams</h1>
                  <p className="text-zinc-600 text-sm mt-1">Connect your Teams channel to receive notifications</p>
                </div>
              </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => router.push('/integrations/teams/help')}
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs px-2 py-1 bg-[#5059C9]/20 text-[#5059C9] rounded-full font-semibold">Quick Setup</span>
              </div>
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.35 }}>
              ...
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.4 }} className="lg:col-span-2">
                <label className="block text-sm font-bold text-zinc-900 mb-2 flex items-center gap-2">
        ...
        >
          <FeatureCard icon={<HiCheckCircle className="w-5 h-5" />} title="Real-time" description="Instant notifications" />
          <FeatureCard icon={<HiLightningBolt className="w-5 h-5" />} title="Reliable" description="99.9% uptime" />
          <FeatureCard icon={<IoCheckmarkDone className="w-5 h-5" />} title="Easy Setup" description="5-min config" />
        </motion.div>
      </div>
    </div>
  );
